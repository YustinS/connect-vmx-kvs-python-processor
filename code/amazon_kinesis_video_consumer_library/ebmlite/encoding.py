...
def encodeId(eid, length=None):
    ...
    if length is not None:
        if length < 1 or length > 4:
            raise ValueError("Cannot encode an ID 0x%0x to length %d" % (eid, length))
...
def encodeUInt(val, length=None):
    ...
    if len(packed) > length:
        raise ValueError(
            "Encoded length (%d) greater than specified length "
            "(%d)" % (len(packed), length)
        )
...
def encodeBinary(val, length=None):
    ...
    if length is None:
        ...
        raise ValueError(
            "Length of data (%d) exceeds specified length (%d)" % (len(val), length)
        )
...
def encodeDate(val, length=None):
    ...
    nanoseconds = (
        delta.microseconds + ((delta.seconds + (delta.days * 86400)) * 1000000)
    ) * 1000
