...
SCHEMA_PATH.extend(
    p
    for p in os.environ.get("EBMLITE_SCHEMA_PATH", "").split(os.path.pathsep)
...
class Element(object):
    ...
    def encode(cls, value, length=None, lengthSize=None, infinite=False):
        ...
        if isinstance(value, (list, tuple)):
            if not cls.multiple:
                raise ValueError(
                    "Multiple %s elements per parent not permitted" % cls.name
                )
            result = bytearray()
            for v in value:
...
class UnknownElement(BinaryElement):
    ...
    def __init__(
        self, stream=None, offset=0, size=0, payloadOffset=0, eid=None, schema=None
    ):
...
class MasterElement(Element):
    ...
    def parseElement(self, stream, nocache=False):
        ...
        try:
            ...
            el = self.schema.UNKNOWN(
                stream, offset, esize, payloadOffset, eid=eid, schema=self.schema
            )
    ...
    def encodePayload(cls, data, length=None):
        ...
        if data is None:
            ...
            raise TypeError("wrong type for %s payload: %s" % (cls.name, type(data)))
        for k, v in data:
    ...
    def encode(cls, data, length=None, lengthSize=None, infinite=False):
        ...
        if isinstance(data, list) and len(data) > 0 and isinstance(data[0], list):
            ...
            for v in data:
                result.extend(
                    cls.encode(
                        v, length=length, lengthSize=lengthSize, infinite=infinite
                    )
                )
            return result
        ...
        return super(MasterElement, cls).encode(
            data, length=length, lengthSize=lengthSize, infinite=infinite
        )
...
class Document(MasterElement):
    ...
    def __init__(self, stream, name=None, size=None, headers=True):
        ...
        if not all(
            (hasattr(stream, "read"), hasattr(stream, "tell"), hasattr(stream, "seek"))
        ):
            raise TypeError(
                "Object %r does not have the necessary stream methods" % stream
            )
    ...
    def __repr__(self):
        ...
        return "<%s %r at 0x%08X>" % (self.__class__.__name__, self.name, id(self))
...
class Schema(object):
    ...
    def __init__(self, source, name=None):
        ...
        if root.tag == "table":
            ...
            raise IOError(
                "Could not parse schema; expected root element "
    ...
    def _parseLegacySchema(self, schema):
        ...
        for el in schema.findall("element"):
            ...
            docs = (
                el.text.strip()
                if isinstance(el.text, (str, bytes, bytearray))
                else None
            )

            if etype is None:
                raise ValueError(
                    'Element "%s" (ID 0x%02X) missing required '
                    '"type" attribute' % (ename, eid)
                )

            if etype not in self.ELEMENT_TYPES:
                raise ValueError(
                    "Unknown type for element %r (ID 0x%02x): %r" % (ename, eid, etype)
                )

            self.addElement(eid, ename, self.ELEMENT_TYPES[etype], attribs, docs=docs)
    ...
    def addElement(self, eid, ename, baseClass, attribs={}, parent=None, docs=None):
        ...
        if eid in self.elements or ename in self.elementsByName:
            ...
            if self.elementInfo[eid] == newatts:
                ...
                raise TypeError(
                    "Element %r (ID 0x%02X) redefined with "
            ...
            if eid is None:
                raise ValueError(
                    "Element definition missing required " '"id" attribute'
                )
            elif not isinstance(eid, int):
                raise TypeError(
                    "Invalid type for element ID: "
                    + "{} ({})".format(eid, type(eid).__name__)
                )

            if ename is None:
                raise ValueError(
                    "Element definition missing required " '"name" attribute'
                )
            elif not isinstance(ename, (str, bytes, bytearray)):
                raise TypeError(
                    "Invalid type for element name: "
    ...
    def __repr__(self):
        try:
            ...
            return "<%s %r from %s>" % (self.__class__.__name__, self.name, source)
        except AttributeError:
            return object.__repr__(self)
    ...
    def verify(self, data):
        ...
        def _crawl(el):
            if isinstance(el, MasterElement):
                ...
                raise NameError("Verification failed, unknown element ID %x" % el.id)
            else:
                _ = el.value
    ...
    if importlib_resources:
        ...
        if isinstance(path, types.ModuleType):
            ...
            path = os.path.dirname(
                importlib.import_module(strpath.strip("{}")).__file__
            )
