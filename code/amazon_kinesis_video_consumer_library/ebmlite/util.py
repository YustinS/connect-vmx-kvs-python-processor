...
def createID(schema, idClass, exclude=(), minId=0x81, maxId=0x1FFFFFFE, count=1):
    ...
    if idc not in ranges:
        raise KeyError(
            "Invalid ID class %r: must be one of %r" % (idClass, list(ranges))
        )

    # Keep range within the one specified and the one imposed by the ID class
    idrange = (max(ranges[idc][0], minId), min(ranges[idc][1], maxId))
    ...
    try:
        ...
        if valid:
            ...
            if not minId <= elementId <= maxId:
                msg = "ID out of range for class %s %s" % (
                    " ABCD"[length],
                    ranges[length - 1],
                )
                valid = False
...
def xmlElement2ebml(xmlEl, ebmlFile, schema, sizeLength=None, unknown=True):
    ...
    try:
        ...
        if eid is None:
            raise NameError(
                "Unrecognized EBML element name with no 'id' "
...
def xml2ebml(xmlFile, ebmlFile, schema, sizeLength=None, headers=True, unknown=True):
    ...
    if xmlRoot.tag not in schema and xmlRoot.tag != schema.document.__name__:
        raise NameError(
            "XML element %s not an element or document in "
    ...
    if xmlRoot.tag == schema.document.__name__:
        for el in xmlRoot:
            numBytes += xmlElement2ebml(
                el, ebmlFile, schema, sizeLength, unknown=unknown
            )
    else:
        numBytes += xmlElement2ebml(
            xmlRoot, ebmlFile, schema, sizeLength, unknown=unknown
        )
...
def pprint(
    ...
    if isinstance(el, core.Document):
        ...
        out.write(
            "%06s %06s %s %s (ID 0x%0X)" % (el.offset, el.size, tab, el.name, el.id)
        )
        if isinstance(el, core.MasterElement):
            ...
            if values:
                if isinstance(el, core.BinaryElement):
                    ...
                    if isinstance(el, core.VoidElement) and void_codec.NAME != "ignore":
                        ...
                        void_codec.encode(
                            el.value, offset=el.offset, indent=indent, stream=out
                        )
