...
def readElementID(stream):
    ...
    if length > 1:
        eid = _struct_uint32_unpack((ch + stream.read(length - 1)).rjust(4, b"\x00"))[0]
    return eid, length
...
def readElementSize(stream):
    ...
    if length > 1:
        size = _struct_uint64_unpack(
            (chr(size).encode("latin-1") + stream.read(length - 1)).rjust(8, b"\x00")
        )[0]
...
def readFloat(stream, size):
    ...
    raise IOError(
        "Cannot read floating point value of length %s; "
