...
def main():
    ...
    argparser.add_argument(
        ...
        "--single",
        action="store_true",
        help="Generate XML as a single line with no newlines or indents",
    )
    argparser.add_argument(
        "-m",
        "--max",
    ...
    with utils.load_files(args, binary_output=args.single) as (schema, out):
        ...
        if args.max:
            root = ebmlite.util.toXml(
                doc, offsets=True, sizes=True, types=True, ids=True, binary_codec=codec
            )
        else:
            root = ebmlite.util.toXml(
                doc,
