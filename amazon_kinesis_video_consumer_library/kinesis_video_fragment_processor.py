...
https://github.com/aws-samples/amazon-kinesis-video-streams-consumer-library-for-python/blob/main/amazon_kinesis_video_consumer_library/kinesis_video_fragment_processor.py
 """
...
class KvsFragementProcessor:
    ...
    def get_fragment_tags(self, fragment_dom):
        """
        Parses a MKV Fragment Doc (of type ebmlite.core.MatroskaDocument) that is 
        returned to the provided callback from get_streaming_fragments() in this 
        class and returns a dict of the SimpleTag elements found.
        ...
        ### Returns:
            ...
            Dictionary of all SimpleTag elements with format -  
                TagName<String> : TagValue <String | Binary>.
        ...
        # For all SimpleTags types (ID: 0x67C8), save for TagName (ID: 0x7373)
        # and values of TagString (ID:0x4487) or TagBinary (ID: 0x4485 )
        simple_tags_dict = {}
    ...
    def get_audio_track_number_from_simple_block(self, mkv_element):
        ...
        if mkv_element.name == "SimpleBlock":
            ...
            if length == 1:
                # removing VINT_MARKER as per 
                # https://datatracker.ietf.org/doc/rfc8794/ paragraph 4
                track_nr = ord(ch) & 127
                ...
                if segment_child.name == "Cluster":
                    ...
                    for cluster_child in segment_child:
                        if cluster_child.name == "SimpleBlock":
                            simple_block_track_nr = (
                                self.get_audio_track_number_from_simple_block(
                                    cluster_child
                                )
                            )
                            i += 1
                            if track_nr == simple_block_track_nr:
                                track_bytearray.extend(
                                    self.get_raw_audio_track_from_simple_block(
                                        cluster_child
                                    )
                                )
    ...
    def save_connect_fragment_audio_track_as_wav(self, fragment_dom, track_nr):
        """
        ...
        return self.get_track_bytearray(fragment_dom, track_nr)

    def save_connect_fragment_audio_track_from_customer(self, fragment_dom):
        """
        ...
        track_number = self.get_track_number_by_name(
            fragment_dom, "AUDIO_FROM_CUSTOMER"
        )
        if track_number:
            return self.save_connect_fragment_audio_track_as_wav(
                fragment_dom, track_number
            )

    def save_connect_fragment_audio_track_to_customer(self, fragment_dom):
        """
